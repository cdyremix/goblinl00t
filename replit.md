...
- Giveaway prizes have three kinds (`prizeKind`): `cs2` (manual streamer delivery via Trade Office), `bot_item` (auto-rolls into winner's inventory; falls back to coin credit if pouch is full), `bot_coins` (credits `prizeBotCoins` directly to the winner via `loot_drops`). Always serialize/deserialize all three new fields (`prizeKind`, `prizeBotCoins`, `prizeBotRarity`). The create form exposes all three as sibling tabs ("CS2 Skin / Loot Drop / Coins"); the `bot_item` ("Loot Drop / 👺") tab labels and copy stay goblin-themed regardless of `botTheme`. The CS2 tab's skin picker remains unchanged — CS2 streamers can switch to the Loot Drop or Coins sibling tabs for bot-driven prizes (which still award CS2-themed items at runtime when `botTheme === "cs2"` because `rollLootDrop` filters `LOOT_TABLE` by the active theme).
