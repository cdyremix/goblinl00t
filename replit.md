...
- `artifacts/goblin-dashboard/src/` — React frontend
  ...
  - `src/components/layout.tsx` — sidebar layout: brand logo → expandable user menu (Collapsible: Account Settings + Sign Out) → main nav (Operations, Loot Hoard, Ledger, Forge, Trade Office). The user menu auto-opens when on `/account`. Account Settings is the only entry point to `/account`; there is a single Sign Out button (inside the user menu). The "Exit Cave" / home link was removed — the brand logo is the only landmark above the user menu.
