...
- `GET /api/maintenance/status` (public, reads optional Clerk session) → `{ enabled, isAdmin }`. `isAdmin` server-resolved, never trusted from client.
