...
**Deferred winner draw** — `EliminationWheel` accepts an optional `onDrawWinner` callback + `drawingWinner` flag. Both the spotlight card on `/giveaway` and `/giveaway/:id` open the wheel with `winner=null` and pass `onDrawWinner={() => endMutation.mutate(...)}`; the wheel renders a "Draw Winner!" CTA in that state. Closing the modal (X / overlay click) without drawing leaves the giveaway active. Once `winner` is set the existing elimination flow runs unchanged.
